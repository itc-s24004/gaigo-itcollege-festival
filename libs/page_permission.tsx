 ...
 * @returns 
 */
export async function checkPagePermission(successCallback: () => Promise<React.ReactNode>, staffOnly: boolean, user?: UserAccount, level?: db_user_level): Promise<React.ReactNode> {
    // レベル指定がない場合は成功とみなす
    if (level === undefined) return await successCallback();
    // ユーザー情報がない場合は認証エラー
    if (user === undefined) return (<ErrorContent title={"認証エラー"} message={"ユーザー情報がありません。ログインしてください。"} />);
    ...
    if (staffOnly) {
        ...
        return (<ErrorContent title={"権限エラー"} message={"このページを表示する権限がありません。"} />);
    }
